...
Future<BitmapDescriptor> getCircleMarkerIcon(
  ...
  debugPrint(
      '[getCircleMarkerIcon] Fetched network image, size: ${imageBytes.lengthInBytes} bytes');
  ...
  debugPrint(
      '[getCircleMarkerIcon] Successfully converted circle image to PNG bytes (length: ${pngBytes.length})');
...
Future<BitmapDescriptor> getPinMarkerIcon(
  ...
  if (cachedBytes != null) {
    debugPrint(
        '[getPinMarkerIcon] Loaded cached pin marker for key: $cacheKey');
    return BitmapDescriptor.bytes(cachedBytes);
  }

  debugPrint(
      '[getPinMarkerIcon] Processing pin marker for URL: $networkImageUrl');
  ...
  final Uint8List circleBytes =
      await createCircleImageBytes(networkImageUrl, circleSize);
  ...
  final ui.Image finalImage =
      await picture.toImage(pinImage.width, pinImage.height);
  final ByteData? byteData =
...
Future<Uint8List> createCircleImageBytes(String url, double size) async {
  ...
  if (cachedBytes != null) {
    debugPrint(
        '[createCircleImageBytes] Loaded cached image for key: $cacheKey');
    return cachedBytes;
  }

  debugPrint(
      '[createCircleImageBytes] Processing image for URL: $url, size: $size');
...
Future<Uint8List> _loadAssetBytes(String assetPath) async {
  ...
  debugPrint(
      '[_loadAssetBytes] Loaded asset: $assetPath, size: ${byteData.lengthInBytes} bytes');
